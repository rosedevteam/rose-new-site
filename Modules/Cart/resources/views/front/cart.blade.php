                                                                     ...
                                                                     @if(!$product->isOnSale())
                                                                         ...
                                                                         @else
                                                                             <div class="d-flex flex-column mt-3">
                                                                                 <del
                                                                                     class="text-danger text-sm">{{ number_format($product->price) }}
                                                                                 ...
                                                                                 <span>{{ number_format($product->price - ($product->price * $cart['discount_percent'])) }}
                                                         <svg class="ts-mr-small" width="14" height="16"
                                                              viewBox="0 0 14 16" fill="none"
                                                              xmlns="http://www.w3.org/2000/svg">
                                                            <path class="text-gray-880 dark:text-white"
                                                                  ...
                                                                  fill="currentColor">
                                                                    ...
                                                                    <span class="d-flex ps-3 pe-3 desc gap-3 justify-content-center">
...
{{--                            @dd(\Modules\Cart\Classes\Helpers\Cart::instance('r-cart'))--}}
                            @php
                                $totalPrice = \Modules\Cart\Classes\Helpers\Cart::all()->sum(function($cart) {
                                    if ($cart['discount_amount'] == 0) {
                                        ...
                                        if (!is_null($cart['product']->sale_price)) {
                                            return ($cart['product']->sale_price - $cart['discount_amount']);
                                        } else {
                                            return  ($cart['product']->price - $cart['discount_amount']);
                                        }
                                    }
                                });
                            @endphp
                            @if($discount = \Modules\Cart\Classes\Helpers\Cart::getDiscount())
                                <span>کد تخفیف فعال : <span class="text-success">{{ $discount->code }}</span> <a
                                        href="#"
                                        class="text-red" id="delete-discount">حذف</a></span>
                                <div>مبلغ تخفیف : <span class="text-success">{{ number_format($discount->amount) }} تومان </span></div>
                            @else
                            <form class="discount-code-wrapper">
                                <input type="text" class="form-control b-none" placeholder="کد تخفیف" name="discount-code" id="discount-code">
                                <button class="btn btn-sm btn-default" id="submit-discount">اعمال</button>
                            </form>
                            @endif
                            ...
                            <a href="#" class="w-100 btn btn-default" disabled>پرداخت</a>
